...
import { RouteHandler, RouteOptions, RouteRequest, RouteResponse } from './api.types';
 ...
 * @template RequestType - Type of the RouteRequest object.
 * @template ResponseType - Type of the RouteResponse object.
...
export const setupRouteHandler =
  ...
  async (req: RequestType, res: ResponseType) => {
    ...
    if (authorization) {
      const auth = authorization(<RouteRequest>req);
    ...
    try {
      let args;
      let input;
      let RouteResponse: RouteResponse = { body: 'OK', status: 200 };
      ...
      if (io?.toResponse) {
        RouteResponse = io.toResponse(output);
      }

      (<BasicResponseType>res).status(RouteResponse.status).send(RouteResponse.body);
    } catch (error) {
      console.log(error);
